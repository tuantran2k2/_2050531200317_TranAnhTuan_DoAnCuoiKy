from controllers._lichSuGiaoDich import create_payment_link, process_payment_status, calculate_total_balance,  fetch_deposit_history , fetch_deposit_history_admin
...
async def deposit(request: DepositRequest, req: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        print(payment_link)
        return {
            "payment_link": payment_link.checkoutUrl,
            "order_id": payment_link.orderCode,
            "maKH": user_id,
        }
...
def get_payment_status(order_id: str, user_id: int, db: Session = Depends(get_db)):
    try:
...
@router.get("/deposit-history/{user_id}")
...
def get_deposit_history(current_user: dict = Depends(get_current_user),db: Session = Depends(get_db)):
    try:
        result = fetch_deposit_history_admin(db)
        if result["transactions"] is None:
            raise HTTPException(status_code=500, detail=result["message"])
        return result
    except Exception as e:
